...
UPDATE_MEM_EVERY = 20                 # how many batches to sample & how often to update the priorities.
UPDATE_MEM_PAR_EVERY = 3000           # how often to update the hyperparameters
EXPERIENCES_PER_SAMPLING = math.ceil(BATCH_SIZE * UPDATE_MEM_EVERY / UPDATE_NN_EVERY) # 1280
...
class Agent():
    ...
    def __init__(self, seed, state_size, action_size, compute_weights = False):
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        self.t_step_nn = (self.t_step_nn + 1) % UPDATE_NN_EVERY #0 -> update every step
        self.t_step_mem = (self.t_step_mem + 1) % UPDATE_MEM_EVERY #1 -> update every 20 step
        self.t_step_mem_par = (self.t_step_mem_par + 1) % UPDATE_MEM_PAR_EVERY #1 -> update every 3000 step
        # Update probability and weigths for all memory.data
        if self.t_step_mem_par == 0:
            self.memory.update_parameters()
        if self.t_step_nn == 0: # every step
            # If enough samples are available in memory, get random subset and learn
            if self.memory.experience_count > EXPERIENCES_PER_SAMPLING:
                # from one sampled batch get (states, actions, rewards, next_states, dones, weights, indices)
                # get one batch of size 64
                sampling = self.memory.sample()
                self.learn(sampling, GAMMA)
        # split into batches
        if self.t_step_mem == 0: #20
    ...
    def act(self, state, eps=0.):
        ...
        if random.random() > eps: #choose maximum probability
    ...
    def learn(self, sampling, gamma):
        ...
        # calculate MSE loss with TD-error
